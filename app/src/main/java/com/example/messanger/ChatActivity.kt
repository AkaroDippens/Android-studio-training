...
private fun fetchMessages(context: Context, currentUserUid: String, friendUid: String, onResults: (List<Message>) -> Unit) {
    ...
    var previousMessages = listOf<Message>()
    ...
    messagesCollection
        ...
        .addSnapshotListener { querySnapshot, error ->
            ...
            querySnapshot?.let { snapshot ->
                ...
                if (results.size > previousMessages.size) {
                    // New message received
                    val newMessages = results.subtract(previousMessages.toSet()).toList()
                    newMessages.forEach { message ->
                        if (message.senderUid != currentUserUid) {
                            fetchFriendNickname(context, message.senderUid) { nickname ->
                                showNewMessageNotification(context, nickname, message.text)
                            }
                        }
                    }
                }
...
private fun showNewMessageNotification(context: Context, senderNickname: String, messageText: String) {
